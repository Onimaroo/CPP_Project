...
> Il faudrait alors chercher dans display_queue l'avion qui a ce numéro de vol.
...
> La fonction-membre timer() défini dans le fichier opengl_interface.cpp y est responsable (elle efface l'avion dans la move_queue)
2. Quelles autres structures contiennent une référence sur un avion au moment où il doit être détruit ?
> La classe Displayable en faisant appel à son destructeur. La classe Terminal contient aussi une référence sur l'avion. 
3. Comment fait-on pour supprimer la référence sur un avion qui va être détruit dans ces structures ?
> On vérifie le booléen renvoyé par chacune des fonctions-membres de ces structures pour qu'elles puissent en quelque sorte "communiquer" entre eux et faire les actions nécéssaires à la suppression de l'avion.
4. Pourquoi n'est-il pas très judicieux d'essayer d'appliquer la même chose pour votre `AircraftManager` ?
> Puisque AircraftManager doit gérer tout ça par lui-même et ne dépend pas des autres classes.
...
> On lui met comme type un unique_ptr de la classe des avions.
...
> On fait en sorte qu'elle soit une sous-classe de DynamicObject.
