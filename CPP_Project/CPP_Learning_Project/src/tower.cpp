...
{
    const auto point = airport.reserve_terminal(aircraft);
    if (!point.first.empty())
    {
        std::cout << "L'avion " << aircraft.get_flight_num() << " a reservé un terminal." << std::endl;
        reserved_terminals.emplace(&aircraft, point.second);
    }
    return point.first;
}
